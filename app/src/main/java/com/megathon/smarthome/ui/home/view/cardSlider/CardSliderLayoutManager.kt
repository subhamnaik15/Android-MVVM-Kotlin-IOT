...
class CardSliderLayoutManager : RecyclerView.LayoutManager, RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    var cardWidth: Int = 0
        private set
    var activeCardLeft: Int = 0
        private set
    var activeCardRight: Int = 0
        private set
    var activeCardCenter: Int = 0
        private set

    var cardsGap: Float = 0.toFloat()
        private set
    ...
    private fun initialize(left: Int, width: Int, gap: Float, updater: ViewUpdater?) {
        ...
        if (this.viewUpdater == null) {
            this.viewUpdater = DefaultViewUpdater()
        }
    ...
    override fun onRestoreInstanceState(parcelable: Parcelable?) {
        if (parcelable is SavedState) {
            val state = parcelable as SavedState?
    ...
    private fun loadViewUpdater(context: Context, className: String?, attrs: AttributeSet): ViewUpdater? {
        ...
        if (className[0] == '.') {
            fullClassName = context.packageName + className
        } else if (className.contains(".")) {
            fullClassName = className
        } else {
            fullClassName = CardSliderLayoutManager::class.java.getPackage()?.getName() + '.'.toString() + className
        }
    ...
    private fun scrollRight(dx: Int): Int {
        ...
        val jDelta = Math.floor((1f * delta.toFloat() * step.toFloat() / cardWidth).toDouble()).toInt()
    ...
    private fun scrollLeft(dx: Int): Int {
        ...
        val step = activeCardLeft / LEFT_CARD_COUNT
        val jDelta = Math.ceil((1f * delta.toFloat() * step.toFloat() / cardWidth).toDouble()).toInt()
    ...
    private fun layoutByCoords() {
        ...
        for (i in 0 until count) {
            val view = getChildAt(i)
            val viewLeft = cardsXCoords.get(getPosition(view!!))
    ...
    companion object {
        ...
        private val DEFAULT_CARD_WIDTH = 148
